...
var ItemType;
(function (ItemType) {
    ItemType[ItemType["PLAYLIST"] = 0] = "PLAYLIST";
    ItemType[ItemType["ALBUM"] = 1] = "ALBUM";
    ItemType[ItemType["TRACK"] = 2] = "TRACK";
    ItemType[ItemType["ARTIST"] = 3] = "ARTIST";
    ItemType[ItemType["CATEGORY"] = 4] = "CATEGORY";
    ItemType[ItemType["EPISODE"] = 5] = "EPISODE";
})(ItemType || (ItemType = {}));
var Direction;
(function (Direction) {
...
figma.ui.onmessage = msg => {
    ...
    function parseItems(jsonString) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield figma.loadFontAsync(interMedium);
    ...
    function drawCover(cover) {
        ...
        const coverRect = figma.getNodeById(id);
    ...
    function getItems(json) {
        ...
        if (json.playlists && json.playlists.length > 0)
            return { data: json.playlists, type: ItemType.PLAYLIST };
        if (json.playlists && json.playlists.items && json.playlists.items.length > 0)
            return { data: json.playlists.items, type: ItemType.PLAYLIST }; // ex. https://developer.spotify.com/console/get-featured-playlists/
        if (json.href && json.href.includes("playlists?"))
            return { data: json.items, type: ItemType.PLAYLIST }; // ex. https://developer.spotify.com/console/get-playlists/
        // Albums
        if (json.albums && json.albums.length > 0)
            return { data: json.albums, type: ItemType.ALBUM }; // ex. https://developer.spotify.com/console/get-several-albums/
        if (json.albums && json.albums.items && json.albums.items.length > 0)
            return { data: json.albums.items, type: ItemType.ALBUM }; // ex. https://developer.spotify.com/console/get-new-releases/
        ...
        if (json.href && json.href.includes("albums?"))
            return { data: json.items, type: ItemType.ALBUM }; // ex. https://developer.spotify.com/console/get-artist-albums/
        // Tracks    
        if (json.tracks && json.tracks.length > 0)
            return { data: json.tracks, type: ItemType.TRACK }; // ex. https://developer.spotify.com/console/get-recommendations/
        ...
        if (json.tracks && json.tracks.href && json.tracks.href.includes("tracks?") && json.tracks.items && json.tracks.items[0].hasOwnProperty("track")) { // ex. https://developer.spotify.com/console/get-playlist/
            let items = [];
            for (let item of json.tracks.items)
                items.push(item.track);
            return { data: items, type: ItemType.TRACK };
        }
        if (json.href && json.href.includes("tracks?"))
            return { data: json.items, type: ItemType.TRACK }; // ex. https://developer.spotify.com/console/get-album-tracks/
        //Artists
        if (json.artists && json.artists.length > 0)
            return { data: json.artists, type: ItemType.ARTIST }; // ex. https://developer.spotify.com/console/get-several-artists/
        if (json.href && json.href.includes("artists?"))
            return { data: json.items, type: ItemType.ARTIST }; // ex. https://developer.spotify.com/console/get-current-user-top-artists-and-tracks?type=artists
        //Categories
        if (json.categories && json.categories.items && json.categories.items.length > 0)
            return { data: json.categories.items, type: ItemType.CATEGORY }; // ex. https://developer.spotify.com/console/get-browse-categories/
        // Nothing
        return null;
    }
};
